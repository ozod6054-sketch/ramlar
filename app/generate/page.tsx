  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({ prompt, style: selectedStyle, size: "1024x1024" }),
      });
        ...
        <div className="max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
            ...
            <div className="flex items-center justify-center gap-2 mt-3">
              ...
              <span className="text-white/60 text-sm">FLUX AI • Hugging Face</span>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="space-y-6">
              <div className="glass rounded-2xl p-6">
                <label className="block text-white/80 text-sm font-medium mb-3">📝 Tavsif (Prompt)</label>
                <textarea
                  value={prompt}
                  onChange={(e) => setPrompt(e.target.value)}
                  placeholder="Mushuk velosipedda tog'lar orasida ketmoqda..."
                  rows={4}
                  className="input-dark w-full resize-none text-sm"
                />
                <p className="text-white/30 text-xs mt-2">Inglizcha yozsangiz natija yaxshiroq bo'ladi</p>
              </div>

              <div className="glass rounded-2xl p-6">
                <label className="block text-white/80 text-sm font-medium mb-3">🎨 Uslub</label>
              ...
              {!session ? (
                <div className="glass rounded-2xl p-6 text-center">
                  ...
                  <p className="text-white/60 text-sm mb-4">Rasm yaratish uchun tizimga kiring</p>
                  <div className="flex gap-3">
                    <Link href="/login" className="flex-1 btn-secondary text-center text-sm py-2.5">Kirish</Link>
                    <Link href="/register" className="flex-1 btn-primary text-center text-sm py-2.5">Ro'yxatdan o'tish</Link>
                  </div>
                </div>
              ) : (
                <button
                  ...
                  className={cn(
                    ...
                    isGenerating || !prompt.trim() ? "bg-white/10 text-white/40 cursor-not-allowed" : "btn-primary hover:scale-[1.02]"
                  )}
                >
                  {isGenerating ? (
                    <><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Yaratilmoqda...</>
                  ) : (
                    <><Wand2 className="w-5 h-5" />Rasm Yaratish<Sparkles className="w-5 h-5" /></>
                  )}
                </button>
              )}
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
                  ...
                  {isGenerating ? (
                    <motion.div key="generating" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 flex flex-col items-center justify-center">
                      <div className="relative w-32 h-32 mb-6">
                        ...
                        <div className="absolute inset-4 rounded-full border-4 border-pink-500/40 animate-spin" style={{ animationDirection: "reverse", animationDuration: "1.5s" }} />
                        <div className="absolute inset-0 flex items-center justify-center">
                          <Sparkles className="w-10 h-10 text-indigo-400 animate-pulse" />
                        </div>
                      </div>
                      <p className="text-white/80 font-medium text-lg mb-2">AI rasm yaratmoqda...</p>
                      <p className="text-white/40 text-sm text-center max-w-xs">"{prompt.slice(0, 50)}{prompt.length > 50 ? "..." : ""}"</p>
                      <div className="mt-6 flex gap-1">
                        {[0, 1, 2, 3, 4].map((i) => (
                          <motion.div key={i} className="w-2 h-2 bg-indigo-500 rounded-full" animate={{ y: [0, -8, 0] }} transition={{ duration: 0.6, repeat: Infinity, delay: i * 0.1 }} />
                        ))}
                      </div>
                    </motion.div>
                  ) : generatedImage ? (
                    <motion.div key="result" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="absolute inset-0">
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                        ...
                        {generationTime && <p className="text-white/40 text-xs mb-3">⚡ {generationTime} soniyada yaratildi</p>}
                        <div className="flex gap-3">
                          <button onClick={handleDownload} className="flex-1 glass py-2.5 rounded-xl text-sm font-medium hover:bg-white/20 transition-colors flex items-center justify-center gap-2">
                            <Download className="w-4 h-4" /> Yuklab olish
                          </button>
                          <button onClick={handleGenerate} className="glass p-2.5 rounded-xl hover:bg-white/20 transition-colors">
                    ...
                    <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 flex flex-col items-center justify-center text-center p-8">
                      <div className="w-20 h-20 rounded-2xl bg-white/5 flex items-center justify-center mb-4">
                        <ImageIcon className="w-10 h-10 text-white/20" />
                      </div>
                      <p className="text-white/40 text-sm">Rasm bu yerda ko'rinadi</p>
                      <p className="text-white/20 text-xs mt-2">Tavsif yozing va "Rasm Yaratish" tugmasini bosing</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
              ...
              {generatedImage && !isGenerating && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex gap-3 mt-4">
                  <button onClick={handleDownload} className="flex-1 glass py-3 rounded-xl text-sm font-medium hover:bg-white/10 transition-colors flex items-center justify-center gap-2">
                    <Download className="w-4 h-4" /> Yuklab olish
                  </button>
                  <button onClick={handleShare} className="flex-1 glass py-3 rounded-xl text-sm font-medium hover:bg-white/10 transition-colors flex items-center justify-center gap-2">
                    <Share2 className="w-4 h-4" /> Ulashish
                  </button>
                  <button onClick={handleGenerate} className="glass px-4 py-3 rounded-xl hover:bg-white/10 transition-colors">
                    <RefreshCw className="w-4 h-4" />
                  </button>
                </motion.div>
              )}
